...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <main className="p-6 md:p-10 lg:p-14 min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">ไม่พบโปรเจกต์นี้</h1>
  ...
  return (
    <main className="p-6 md:p-10 lg:p-24 min-h-screen bg-white text-gray-900 font-sans antialiased relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
            <div className="flex flex-col">
              ...
              <div className="text-sm text-gray-600 mb-8">
                <p className="mb-1"><span className="font-semibold text-gray-900">Role:</span> {project.role}</p>
                <p><span className="font-semibold text-gray-900">Status:</span> {project.status}</p>
                <p className="mb-1"><span className="font-semibold text-gray-900">Platform:</span> {isMobile ? "📱 Mobile App" : "💻 Web Application"}</p>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.techStack.map((tech: string, index: number) => (
                    <span
                      key={index}
                      className="inline-block bg-gray-900 text-white text-sm font-medium px-4 py-2 rounded-lg transition-all hover:bg-gray-800 hover:scale-105 cursor-default"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="bg-gray-50 rounded-2xl p-6 md:p-8 border border-gray-200 h-fit">
            <div className="flex items-center justify-end mb-4">
              <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 bg-white border border-gray-200 text-gray-900 text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                <svg className="w-3.5 h-3.5 text-gray-900" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
                GitHub
              </a>
            </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {project.features.map((feature: any, index: number) => (
              <div key={index} className="bg-gray-50 border border-gray-200 rounded-2xl p-6 hover:shadow-md transition-shadow duration-300">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs text-gray-600">{feature.category}</span>
                  <span className="inline-flex items-center gap-1.5 bg-white border border-gray-200 text-gray-900 text-xs font-semibold px-2.5 py-1 rounded-lg">
                    {icons[feature.icon]}
                    {feature.badge}
                  </span>
                </div>
          ...
          <div className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">ผู้ใช้งานและบทบาทในระบบ</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {project.actors.map((actor, index) => (
                <div
                  key={index}
                  className="relative bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group overflow-hidden"
                >
                  <div className="absolute -top-8 -right-8 w-24 h-24 bg-gradient-to-br from-blue-100 to-transparent rounded-full blur-2xl group-hover:scale-125 transition-transform duration-500" />

                  <div className="relative flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-gray-50 to-white border border-gray-200 flex items-center justify-center text-3xl mb-4 shadow-sm group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
                      {actor.emoji}
                    </div>

                    <h4 className="text-base font-bold text-gray-900 mb-2">{actor.role}</h4>
